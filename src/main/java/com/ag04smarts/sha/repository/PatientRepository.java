...
public interface PatientRepository extends JpaRepository<Patient, Long> {
    ...
    @Query(value = " SELECT * FROM PATIENT p " +
        ...
        "LEFT JOIN medical_record_symptom as mrs ON pmr.patient_medical_record_id = mrs.patient_medical_record_id " +
        "LEFT JOIN symptom as s ON mrs.symptom_id = s.symptom_id" +
        "AND UPPER(s.description) = UPPER('Coughing') OR UPPER(s.description) = UPPER('Fever') ", nativeQuery = true)
